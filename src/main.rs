			...
			match buf.read_line(&mut result).await {
				...
				Ok(_) => {
					...
					if result.starts_with("Content-Length") {
						content_length = usize::from_str(&*result
							.replace("Content-Length: ", "")
							.replace("\r\n", "")).unwrap();
					} else if result.contains("HTTP/") {
						let mut first_line = result.split_whitespace().collect::<Vec<&str>>();
						key = first_line.get(1).unwrap().parse::<String>().unwrap().replace("/", "");
				...
				Err(e) => {
					eprintln!("{}", e);
					buf.write_all(b"HTTP/1.1 500 Internal Server Error").await?;
					break 'outer;
				}
			}
		}
			...
			HttpMethods::GET => {
				match query_db(&db, key).await {
					...
					Some(entry) => {
						...
						buf.write_all(response.as_bytes()).await?;
					}
				}
			}
			HttpMethods::POST => {
				...
				if let Ok(body) = json::parse(&*raw_body) {
					...
					eprintln!("[{}]: Invalid JSON", socket_addr);
					buf.write_all(b"HTTP/1.1 400 Bad Request").await?;
				}
			}
			HttpMethods::DELETE => {
...
#[async_recursion]
async fn update_db(db: &Arc<Mutex<HashMap<String, Object>>>, data: JsonValue, key: String, socket_addr: SocketAddr) {
	match data {
		JsonValue::Object(data) => {
			let mut db = db.lock().await;
			db.insert(key, data);
		}
		JsonValue::Array(arr) => for element in arr {
